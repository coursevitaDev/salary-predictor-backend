...
def extract_missing_skills_and_certificates_with_gemini(user_data):
    """
    ...
    print(user_data)
    user_certifications = user_data.get("certifications", [])
    user_experiences = user_data.get("experiences", [])
    job_role = user_data.get("job_role", "")


    prompt = f"""
    Given the job role: {job_role} and the list of user-skills, job-relevant skills, certifications and professional experiences provided, identify the missing skills of the user for the job.
    determine whether each certification and experience is relevant to the job role.
    ...
    {{
      "certifications_relevance": {{"Certification1": true/false, "Certification2": true/false}},
      "experiences_relevance": {{"Experience1": true/false, "Experience2": true/false}}
    }}
    """
    ...
    for api_key in api_keys:
        try:
            ...
            return {
                "missing_skills":relevance_data.get(" missing_skills", {}),
...
def search():
    ...
    for i in range(len(indices[0])):
        ...
        if category_index < len(index_to_category):
            ...
            if os.path.exists(category_skills_file):
                ...
                results.append({"role_title":role_title,"exp":exp,"category": category,"base_pay":base_pay,"salary_estimate":salary_estimate, "matching_skills": matching_skills,"skill_match":skill_match, "user_skill_score":skill_score_category,"missing_skills":missing_skills})
    ...
    for x in range(len(results)):  # Use range(len(results))
        final.append({
            ...
            "Skill Score": round(results[x]["user_skill_score"]*100,2),
            "Experience": results[x]["exp"],
